...
#
# PxeBaseCode.py is free software: you can redistribute it and/or
...
if EFIPY_MDE_CPU_TYPE == EFIPY_MDE_CPU_IA32:
  ...
  EFI_PXE_CLIENT_SYSTEM_ARCHITECTURE    = 0x0002
...
class EFI_PXE_BASE_CODE_DHCPV4_PACKET (Structure):
  _fields_ = [
    ...
    ("BootpSeconds;       ", UINT16),
    ("BootpFlags;         ", UINT16),
    ("BootpCiAddr[4];     ", UINT8 * 4),
    ("BootpYiAddr[4];     ", UINT8 * 4),
    ("BootpSiAddr[4];     ", UINT8 * 4),
    ("BootpGiAddr[4];     ", UINT8 * 4),
    ("BootpHwAddr[16];    ", UINT8 * 16),
    ("BootpSrvName[64];   ", UINT8 * 64),
    ("BootpBootFile[128]; ", UINT8 * 128),
    ("DhcpMagik;          ", UINT32),
    ("DhcpOptions[56];    ", UINT8 * 56)
  ]
